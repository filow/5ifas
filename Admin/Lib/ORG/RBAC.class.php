 ...
 * 基于角色的数据库方式验证类
 *
 * 2013年8月28日11:19:33   为适配本项目的模式,对level级进行了修改
 * 
 +------------------------------------------------------------------------------
 * @category   ORG
 * @package  ORG
...
class RBAC {
        ...
        if( C('USER_AUTH_ON') ){
            ...
            if((!empty($_module['no']) && !in_array(strtoupper(MODULE_NAME),$_module['no'])) || (!empty($_module['yes']) && in_array(strtoupper(MODULE_NAME),$_module['yes']))) {
				if("" != C('REQUIRE_AUTH_ACTION')) {
					//需要认证的操作
                    //2013年8月28日修改,以使用MODULE_NAME/ACTION_NAME方法定义无需/需要验证的方法
                    $tmp=explode(',',strtoupper(C('REQUIRE_AUTH_ACTION')));
                    foreach($tmp as $k => $v){
                        $tmp[$k]=trim($v);
                    }
					$_action['yes'] = $tmp;
					...
					//无需认证的操作
                    $tmp=explode(',',strtoupper(C('NOT_AUTH_ACTION')));
                    foreach($tmp as $k => $v){
                        $tmp[$k]=trim($v);
                    }
					$_action['no'] = $tmp;
				...
				//检查当前操作是否需要认证
                $current_action=strtoupper(MODULE_NAME."/".ACTION_NAME);
				if((!empty($_action['no']) && !in_array($current_action,$_action['no'])) || (!empty($_action['yes']) && in_array($current_action,$_action['yes']))) {
					return true;
				}else {
					return false;
				}
     ...
     * 取得当前认证号的所有权限列表
     +----------------------------------------------------------
     * 2013年8月28日  删除项目分组的信息,以使其可以在普通分组模式下运行
     +----------------------------------------------------------
    ...
    static public function getNav() {
        ...
        $sql    =   "select node.id,node.name,node.title from ".
                ...
                $table['access']." as access ,".
                $table['node']." as node ".
                "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.status=1 order by sort";
        $modules =   $db->query($sql);
        // 依次读取模块的操作权限
        foreach($modules as $key=>$module) {
            $moduleId    =   $module['id'];
            $moduleName = $module['name'];
            $sql    =   "select node.title,node.name from ".
                $table['role']." as role,".
                $table['user']." as user,".
                $table['access']." as access ,".
                $table['node']." as node ".
            ...
            $action = array();
            foreach ($rs as $a){
                $action[$a['name']]  =   $a['title'];
            }
            $action['MODULE_TITLE'] = $module['title'];
            // 和公共模块的操作权限合并
