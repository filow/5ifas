...
class AmountinfoModel  extends Model {
    ...
    function czid($orderid) {
        ...
        $amount = 0;
        if ($result = $amountinfo->where(array("orderid" => $orderid, "type" => 0))->save(array("suctime" => $time, "type" => 1, "operator" => $_SESSION['username']))) {
            if ($result = $amountinfo->where(array("orderid" => $orderid, "type" => 4))->save(array("suctime" => $time, "type" => 2, "operator" => $_SESSION['username']))) {
                $result = $amountinfo->where(array("orderid" => $orderid, "type" => 2))->find();
                $amount = $result['amount'];
            }
            $result2 = $amountinfo->where(array("orderid" => $orderid, "type" => 1))->find();
            $dj1 = $info->where(array("type" => 4, "value2 <=" => $result2['amount']))->order("value2 desc ")->select();
